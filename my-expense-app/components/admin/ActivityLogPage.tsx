...
const transformStats = (apiStats: ActivityStats | null | undefined): QuickStats => {
  ...
  return {
    activeUsers: apiStats.active_users_today ?? 0,
    actionsToday: apiStats.today_count ?? 0,
    warnings: apiStats.warning_count ?? 0,
    errors: apiStats.error_count ?? 0,
    topUsers: [], // Backend doesn't provide this yet
  };
};
...
export default function ActivityLogPage() {
    ...
    try {
      ...
      if (response.success && response.data?.logs) {
        ...
        if (transformedLogs.length > 0) {
          setLastLogId(parseInt(transformedLogs[0].id));
        }
      }
    } catch (error) {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (response.success && response.data?.logs && response.data.logs.length > 0) {
          const newLogs = response.data.logs.map(transformApiLog);
          setLogs(prev => [...newLogs, ...prev].slice(0, 200));
          setLastLogId(response.data.last_id);
        }
  ...
  }, [isLive, refreshInterval, lastLogId, fetchStats]);
