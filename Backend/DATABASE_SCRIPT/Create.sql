...
IF EXISTS (SELECT * FROM sys.objectS WHERE object_id = OBJECT_ID('notifications') AND type in (N'U'))
...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID('debt_payments') AND type in (N'U'))
...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID('transfer') AND type in (N'U'))
DROP TABLE transfer; 
GO
...
CREATE TABLE accounts(
    ...
    is_include_in_total BIT , --- có bao gồm trong tổng tài sản hay không ( ví dụ : thẻ tín dụng, khoả vay nợ trong tính vào tổng tài san vì nó là khoản nợ , không phải tài sản ) ---
    created_at DATETIME DEFAULT GETDATE() , 
    updated_at DATETIME DEFAULT GETDATE() , 
...
CREATE TABLE recurring_transactions(
    ...
    CONSTRAINT fk_recurring_account FOREIGN KEY KEY(account_id) REFERENCES accounts(account_id) ON DELETE CASCADE , 
    CONSTRAINT fk_recurring_category FOREIGN KEY (categoy_id) REFERENCES categories(category_id) ON DELETE CASCADE ,
...
--- Tạo bảng transaction transaction( lưu trữ lịch sử giao dịch )
CREATE TABLE transactions(
    ...
    CONSTRAINT fk_transaction_account FOREIGN KEY (category_id) REFERENCES categories(category_id) ON DELETE CASCADE , 
...
CREATE TABLE transfers(
    ...
    CONSTRAINT fk_transfer_from_account FOREIGN KEY (from_account_id) REFERENCES accounts(account_id) ON DELETE CASCADE , 
    CONSTRAINT fk_transfer_to_account FOREIGN KEY (to_account_id) REFERENCES accounts(account_id) ON DELETE CASCADE 
...
CREATE TABLE debts(
    ...
    CONSTRAINT check_status CHECK (status IN ('active', 'completed', 'overdue'))
)
GO 
...
---  TẠO BẢNG SAVING GOALS ( MUỤC TIÊU TIẾT KIỆM ) ---- 
...
CREATE TABLE notification(
    ...
    CONSTRAINT fk_notifiaction_user FOREIGN KEY (user_id) REFERENCES users(user_id) ON DELETE CASCADE 
)
...
CREATE TABLE user_setting (
    ...
    enabale_notification BIT DEFAULT 1 , 
...
(
    ...
    CONSTRAINT fk_share_owner FOREIGN KEY (owner_id) REFERENCES users(user_id) ON DELETE CASCADE , 
...
-- 12. user_setting & shared_access
-- Thường chỉ query theo user_id → index đơn giản đủ
